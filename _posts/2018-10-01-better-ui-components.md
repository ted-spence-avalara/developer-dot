...
* **S**eattle - that's where we are
* **s**tandards - we always go for standards until we actually need to bring in a dependency
* **s**emantic - tags with attributes are much more semantic than a span with a class list
* **s**mall - basic HTML and CSS can take you very far without needing something like React
* **s**hared - we have over 20 web apps and three times as many developers that need a common set of custom UI components
