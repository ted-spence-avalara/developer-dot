...
The warning means that AvaTax used the default <a href="https://en.wikipedia.org/wiki/International_System_of_Units" target="_blank">Système Internationale d'Unités</a>, or SI Unit, for calculating screen size. The SI Unit for length is the meter/metre. A screen size of 15 meters is rather large and maybe more than we intended. But how do we tell AvaTax which unit we mean? By adding our ScreenSize parameter with a suffix of “.UOM” AvaTax will use the unit you designate, as long its `measurementTypeCode` matches the MeasurementType in the message. 

`/api/v2/definitions/unitofmeasurements`

Setting Up Your Parameters
First we need to know what parameters and units to use for our transaction. Let’s try a computer system with monitor and check what UOM are available using the following:

`/api/v2/definitions/unitofmeasurements?$filter=measurementTypeCode%20EQ%20Length`

The example’s filter checks for UOMs related to length, which yields a fuller version of this abridged response:
```json
   …
    {
      "id": 139,
...
For our computer system with monitor example, we need to populate this tuple of parameters with the relevant data: 
```json
"parameters": {           
          "ScreenSize": "100",
          "ScreenSize.UOM": "Inch"
...
When we put in the correct parameters and measurements, we can expect a response like the truncated one below:
```json
{
  "id": 12345678,
...
When populating your line object, the parameters are case-sensitive, so be sure to follow the exact spelling the endpoint gives you. Consequently, when searching for information in the definitions endpoints, try changing the capitalization if you aren’t finding what you need. 
